...
found:
  ...
  p->memory_limit = 0;    // 메모리 최대 할당값 0으로 초기화
  p->stack = 1;           //스택개수 1개로 초기화
  ...
  *(uint*)sp = (uint)trapret;
  ...
  //
  memset(&p->stacklist, 0, sizeof p->stacklist);
  //
...
fork(void)
{
  ...
  if(curproc->tid > 0){
    np->pgdir = copyuvm(curproc->pgdir, curproc->main->sz);
  }
  else{
...
//Process memory limitation 특정 프로세스에 대해 할당받을 수 있는 메모리 최대치를 제한
...
setmemorylimit(int pid, int limit)
{
  ...
  if(p == &ptable.proc[NPROC]){
    cprintf("setmemroy ptable.proc");
    release(&ptable.lock);
    return -1;
  }

  if(limit < 0 || limit < p->sz || limit < p->memory_limit){
    cprintf("setmemory out of bound");
    release(&ptable.lock);
    return -1;
  }
...
print_pinfo(void){
	...
	for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
		if(p->state!=RUNNING)
		    continue;
		cprintf("process name : %s pid : %d stack page : %d memory size : %d memory limit : %d\n", p->name, p->pid, p->stack, p->sz, p->memory_limit);
	}
	return 0;
}
...
thread_create(thread_t *thread, void* (*start_routine)(void *), void *arg) {
  ...
  *np->tf = *curproc->tf;
  ...
  np->tf->eax = 0;
  np->pid = curproc->pid;
  np->main = curproc; np->tid = nexttid++; 
  release(&ptable.lock);
