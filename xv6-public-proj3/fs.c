...
uint *da, *ta;
struct buf *bp;
struct buf *dbp, *tbp;    //double,triple indirect 사용을 위해 추가
...
//symlink
int symlink(struct inode *dp, char *name, uint inum)
{
  int off;
  struct dirent de;
  struct inode *ip;
...
namex(char *path, int nameiparent, char *name)
{
  ...
  while((path = skipelem(path, name)) != 0){
    ...
    {
      if(readi(next, buff, 0, next->size) != next->size ||next->size >= sizeof(buff)) 
      {
        iunlockput(next);
        iput(ip);
        return 0; 
      ...
      iunlockput(next);
      next = namex((char*)next->addrs, 0, filename);
    }
    else iunlock(next);
    iput(ip);
    ip = next;
